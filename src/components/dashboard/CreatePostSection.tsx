...
import { Sparkles, Upload, Loader2, RefreshCw, X, Image as ImageIcon } from "lucide-react";
...
export const CreatePostSection = ({ onPostGenerated, onRegenerateCallback }: CreatePostSectionProps) => {
  ...
  const { templates, isLoading: templatesLoading, getTemplatePreviewUrl } = useTemplates();
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="glass rounded-2xl p-6 space-y-6">
        ...
        <div className="space-y-2">
          <Label>Plantilla de imagen (opcional)</Label>
          <Select 
            value={templateId?.toString()} 
            onValueChange={(val) => setTemplateId(val === "none" ? undefined : Number(val))} 
          ...
          {templates.length === 0 && !templatesLoading && (
            <p className="text-xs text-muted-foreground">
              No tienes plantillas. Puedes crear plantillas en la sección de gestión de plantillas.
            </p>
          )}
          {templateId && templates.find(t => t.id === templateId) && (
            <div className="mt-3 p-3 border rounded-lg bg-background">
              ...
              <div className="relative w-full max-w-[200px] aspect-square overflow-hidden rounded-md border">
                <img
                  src={getTemplatePreviewUrl(templates.find(t => t.id === templateId)!.storage_path)}
                  alt={templates.find(t => t.id === templateId)!.template_name}
